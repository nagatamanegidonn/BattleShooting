...
void Player::DrawUI(int plyarNo)
{
	...
	{
		DrawRotaGraph(cx2 + (plyNum * hpImageSize * i), 30, 1.0f, 0.0f, hpFreamH_, true, plyFlag);
	}
	for (int i = 0; i < playerHp_; i++)
	{
		DrawRotaGraph(cx2 + (plyNum * hpImageSize * i), 30, 1.0f, 0.0f, hpGaugeH_, true, plyFlag);
	}
...
void Player::UpdateReload(void)
{
	...
	if (inputController_->IsNew(InputController::KEY::RELOAD) && (shotMagazine_ < shotMagazineMax_))
	{
		reloadTime_ -= SceneManager::GetInstance().GetDeltaTime();

		if (reloadTime_ <= 0.0f)
		{
			reloadTime_ = reloadSet_;
...
void Player::Move(void)
{
	...
	//進行方向が前か後か
	if (inputController_->IsNew(InputController::KEY::MOVE_FORWARD)) { forward = transform_.GetForward(); }
	else if (inputController_->IsNew(InputController::KEY::MOVE_BACK)) { forward = transform_.GetBack(); }
...
void Player::Turn(VECTOR axis)
{
	...
	if (inputController_->IsNew(InputController::KEY::LEFT)) { direction_.y = -ROT_POW; }
	if (inputController_->IsNew(InputController::KEY::RIGHT)) { direction_.y = ROT_POW; }

	VECTOR addAxis = direction_;

	if (inputController_->IsNew(InputController::KEY::MOVE_BACK))
	{
		addAxis.y *= -1;
	}
...
void Player::ProcessShot(void)
{
	...
	//弾のリロード
	if (inputController_->IsTriggered(InputController::KEY::RELOAD) && (shotMagazine_ < shotMagazineMax_))
	{
		...
		//弾を１発装填するにかかる時間を設定
		reloadSet_ = TIME_RELOAD_RATE + (RELOAD_TIME_LOW * ((float)shotMagazine_ / (float)shotMagazineMax_));
		reloadTime_ = reloadSet_;
